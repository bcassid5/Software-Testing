...
var getBalanceFactor=function (account ){
    ...
    if (account.balance < -100 || account .balance > 1000000)
        factor = -1;
...
var creditStatus=function (account,creditCheckMode){
    ...
    if (account.creditScore < 0  || account.creditScore >800)
        return "invalid";
...
var orderHandling=function(account ,product,inventory,inventoryThreshold,creditCheckMode){
    ...
    if (aStautus==="invalid"||cStatus==="invalid" ||pStatus==="invalid" )
        return "invalid";
    if ((aStautus==="very good")|| (aStautus==="good" && cStatus==="good")||(aStautus!="good" && cStatus==="good" && pStatus==="available"))
        return "accepted";
    else if ((aStautus==="good" && cStatus==="bad")||(aStautus==="fair" && cStatus==="bad" && pStatus==="available"))
        return "underReview";
    else if ((aStautus==="fair" && cStatus==="good" && pStatus!="available") ||(aStautus==="poor" && cStatus==="good" && pStatus==="limited"))
        return "waiting";
    else if ((aStautus==="fair" && cStatus==="bad" && pStatus!="available")||(aStautus==="poor" && cStatus==="good" && pStatus==="soldout")||(aStautus==="poor" && cStatus==="bad" ))
        return "rejected"
}